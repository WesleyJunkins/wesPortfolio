...
// Keep track of all stars
const stars = [];
const maxDistance = 1000; // Maximum distance from the camera before a star is removed
...
function animate() {
    ...
    if (responsivenessCounter % 10 == 0) {
        for (let i = 0; i < 3; i++) {
            const geometry = new THREE.SphereGeometry(1.50, 24, 24);
            const material = new THREE.MeshStandardMaterial({ color: Math.random() * 0xffffff });
            const star = new THREE.Mesh(geometry, material);
            const [x, y, z] = Array(3).fill().map(() => THREE.MathUtils.randFloatSpread(1000));
            star.position.set(x + camera.position.x, y + camera.position.y, z + camera.position.z);
            scene.add(star);
            stars.push(star);
        }
    }

    // Check and remove stars that exceed the maximum distance from the camera
    for (let i = stars.length - 1; i >= 0; i--) {
        const star = stars[i];
        const distance = camera.position.distanceTo(star.position);
        if (distance > maxDistance) {
            scene.remove(star); // Remove star from the scene
            stars.splice(i, 1); // Remove star from the array
        }
    }
